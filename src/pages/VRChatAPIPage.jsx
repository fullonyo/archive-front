...
const VRChatAPIPage = () => {
  ...
  // Estados para gerenciar filtros e visualização dos amigos
  const [friendsFilter, setFriendsFilter] = useState({
    search: '',
    status: 'all', // all, online, offline, busy, away
    location: 'all', // all, in-world, private, offline
    sortBy: 'name', // name, status, last-seen, activity
  ...
  const ModernFriendCard = ({ friend, index, viewMode = 'grid', cardDensity = 'comfortable', enableAnimations = true }) => {
    ...
    // Animação de entrada escalonada baseada nas configurações
    const cardDelay = enableAnimations ? index * 50 : 0
    
    // Configurações de tamanho baseadas na densidade
    const sizeConfig = {
    ...
    const config = sizeConfig[cardDensity] || sizeConfig.comfortable
    ...
    return (
      ...
      >
          ...
          <div className={`flex items-start ${config.spacing}`}>
            ...
            <div className="relative flex-shrink-0">
              <div className={`
                ...
                ${enableAnimations ? 'group-hover:scale-110 transition-transform duration-300' : ''}
              `}>
                <img
                  src={friend.userIcon}
        ...
        {friend.location && !friend.location.includes('offline') && cardDensity !== 'dense' && (
          <div className={`${config.padding} pb-2`}>
            <div className="bg-gray-700/50 rounded-lg p-3">
              ...
              {!friend.location.includes('private') && cardDensity === 'comfortable' && (
        ...
        {/* Footer com Quick Actions e Info */}
        <div className={config.padding}>
          {/* Última Atividade */}
          {lastActivity && cardDensity !== 'dense' && (
            <div className="bg-gray-700/30 rounded-lg p-2 mb-3">
              ...
              <p className="text-xs text-gray-300 mt-1">
                ...
                {lastActivity.type === 'description_change' && 'Mudou descrição'}
              </p>
            </div>
          )}
          ...
          {cardDensity === 'comfortable' && (
            <div className={`
              ...
              ${isHovered && enableAnimations ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}
  ...
  const getFilteredAndSortedFriends = () => {
    if (!dashboardData?.friends?.friends) return []
    
    let filtered = dashboardData.friends.friends.filter(friend => {
            ...
            <AnimatePresence mode="wait">
              ...
              {activeSection === 'friends' && (
                <motion.div
                  ...
                  className="space-y-6"
                >
                  {/* Interface moderna da seção de amigos */}
                  <div className="space-y-6">
                    ...
                    <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
                      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                        <div>
                          <h2 className="text-2xl font-bold text-white mb-2 flex items-center space-x-3">
                            ...
                            <span>Amigos {dashboardData?.friends?.friends ? `(${getFilteredAndSortedFriends().length}/${dashboardData.friends.friends.length})` : ''}</span>
                          </h2>
                          <p className="text-gray-400 flex items-center space-x-4">
                            <span className="flex items-center space-x-1">
                              <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                              <span>{getFilteredAndSortedFriends().filter(f => f.status !== 'offline').length} online</span>
                            </span>
                            <span className="flex items-center space-x-1">
                              <div className="w-2 h-2 bg-gray-500 rounded-full"></div>
                              <span>{getFilteredAndSortedFriends().filter(f => f.status === 'offline').length} offline</span>
                            </span>
                          </p>
                        </div>
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div className="flex items-center space-x-2 bg-gray-700 rounded-lg p-1">
                            ...
                            <button
                              onClick={() => setFriendsFilter(prev => ({ ...prev, viewMode: 'compact' }))}
                              className={`p-2 rounded transition-colors ${
                                friendsFilter.viewMode === 'compact' ? 'bg-orange-600 text-white' : 'text-gray-400 hover:text-white'
                              }`}
                      ...
                      {/* Navegação Rápida para Visualizações */}
                      <div className="mt-4 flex flex-wrap gap-2">
                        <button
                          onClick={() => setActiveVisualization('timeline')}
                          className="bg-gray-700 hover:bg-gray-600 text-white px-3 py-1 rounded-lg text-sm transition-colors flex items-center space-x-2"
                        >
                          <ClockIcon className="w-4 h-4" />
                          <span>Timeline</span>
                        </button>
                        <button
                          onClick={() => setActiveVisualization('flow')}
                          className="bg-gray-700 hover:bg-gray-600 text-white px-3 py-1 rounded-lg text-sm transition-colors flex items-center space-x-2"
                        >
                      ...
                      {/* Estatísticas Rápidas e Configurações de Performance */}
                        ...
                        <div className="mt-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                          <div className="flex flex-wrap gap-3 text-sm">
                            ...
                            <div className="bg-blue-600/20 text-blue-400 px-3 py-1 rounded-full flex items-center space-x-2">
                              ...
                              <span>{getFilteredAndSortedFriends().filter(f => f.location && !f.location.includes('offline') && !f.location.includes('private')).length} em mundos</span>
                            </div>
                            <div className="bg-purple-600/20 text-purple-400 px-3 py-1 rounded-full flex items-center space-x-2">
                              <EyeSlashIcon className="w-4 h-4" />
                              <span>{getFilteredAndSortedFriends().filter(f => f.location?.includes('private')).length} em privado</span>
                            </div>
                          ...
                          <div className="flex items-center space-x-3">
                            <div className="flex items-center space-x-2">
                              ...
                              <button
                                onClick={() => setFriendsViewSettings(prev => ({ ...prev, enableAnimations: !prev.enableAnimations }))}
                                className={`w-10 h-5 rounded-full transition-colors ${
                                  friendsViewSettings.enableAnimations ? 'bg-orange-600' : 'bg-gray-600'
                                } relative`}
                              >
                                <div className={`w-4 h-4 bg-white rounded-full absolute top-0.5 transition-transform ${
                                  friendsViewSettings.enableAnimations ? 'translate-x-5' : 'translate-x-0.5'
                                }`}></div>
                              </button>
                            </div>
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              <select
                                value={friendsViewSettings.cardDensity}
                                onChange={(e) => setFriendsViewSettings(prev => ({ ...prev, cardDensity: e.target.value }))}
                    ...
                    {/* Grid de Amigos com Performance Otimizada */}
                    {getFilteredAndSortedFriends().length > 0 ? (
                      <div className={`
                        ${friendsFilter.viewMode === 'grid' && friendsViewSettings.cardDensity === 'comfortable' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6' :
                          ...
                          friendsFilter.viewMode === 'list' ? 'space-y-3' :
                          'grid grid-cols-1 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-2'}
                        transition-all duration-300
                      `}>
                        <AnimatePresence mode="popLayout">
                          {getFilteredAndSortedFriends().map((friend, index) => (
                            <ModernFriendCard 
                              key={friend.id} 
                              friend={friend} 
                              index={index}
                              viewMode={friendsFilter.viewMode}
                              cardDensity={friendsViewSettings.cardDensity}
                              enableAnimations={friendsViewSettings.enableAnimations}
                            />
                          ))}
                        </AnimatePresence>
                      </div>

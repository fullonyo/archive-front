            ...
            >
              <div className="flex items-center gap-4 mb-4">
                <div className="p-3 bg-red-500/20 rounded-full">
                  <TrashIconSolid className="w-6 h-6 text-red-400" />
                </div>
                <div>
...
const AssetManagementSection = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    loadStats()
    loadAssets(1, {})
  }, [loadCategories, loadStats, loadAssets])
  ...
  if (!hasPermission('delete_assets')) {
    return (
      <div className="text-center text-slate-400 py-12">
        ...
        <p>Você não tem permissão para deletar assets.</p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: 'Inativos', value: stats.inactive, color: 'text-red-400', bgColor: 'bg-red-400/20' }
        ].map((stat, index) => (
          <div 
            key={stat.label}
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        >
          Todos os Assets ({stats.total || 0})
        </button>
        <button
          onClick={() => {
            const activeFilters = { isActive: 'true' }
            setFilters(activeFilters)
            loadAssets(1, activeFilters)
          }}
          className="px-4 py-2 bg-green-600/20 hover:bg-green-600/30 border border-green-600/30 hover:border-green-600/50 rounded-lg text-green-400 font-medium transition-all duration-200"
        >
          Assets Ativos ({stats.approved || 0})
        </button>
        <button
          onClick={() => {
            const inactiveFilters = { isActive: 'false' }
            setFilters(inactiveFilters)
            loadAssets(1, inactiveFilters)
          }}
          className="px-4 py-2 bg-red-600/20 hover:bg-red-600/30 border border-red-600/30 hover:border-red-600/50 rounded-lg text-red-400 font-medium transition-all duration-200"
        >
          Assets Inativos ({stats.inactive || 0})
        </button>
      </div>
